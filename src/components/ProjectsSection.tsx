...
const ProjectCard = ({ project }: { project: typeof projects[0] }) => {
  return (
    <Card className="overflow-hidden transition-all hover:shadow-lg hover:-translate-y-1 dark:bg-gray-800 dark:border-gray-700">
      <div className={`h-36 sm:h-48 ${project.image} flex items-center justify-center`}>
        <span className="text-2xl sm:text-3xl font-bold text-primary/40 dark:text-blue-400/40">{project.id}</span>
      </div>
      <CardHeader className="p-3 sm:p-4">
        <CardTitle className="text-base sm:text-lg dark:text-white">{project.title}</CardTitle>
        <CardDescription className="text-xs sm:text-sm dark:text-gray-400">{project.category}</CardDescription>
      </CardHeader>
      <CardContent className="p-3 sm:p-4 pt-0">
        <p className="text-xs sm:text-sm text-muted-foreground dark:text-gray-300">{project.description}</p>
      </CardContent>
      <CardFooter className="p-3 sm:p-4">
        <Button variant="outline" size="sm" className="w-full text-xs sm:text-sm dark:text-gray-300 dark:border-gray-600 dark:hover:bg-gray-700">
...
const ProjectsSection = () => {
  return (
    <section id="projetos" className="py-16 sm:py-20 min-h-screen dark:bg-gray-900">
        ...
        <Tabs defaultValue="all" className="w-full">
          <div className="flex justify-center mb-8 sm:mb-12">
            <TabsList className="dark:bg-gray-800">
              <TabsTrigger value="all" className="text-xs sm:text-sm dark:data-[state=active]:bg-blue-600 dark:data-[state=active]:text-white">Todos</TabsTrigger>
              <TabsTrigger value="frontend" className="text-xs sm:text-sm dark:data-[state=active]:bg-blue-600 dark:data-[state=active]:text-white">Front-end</TabsTrigger>
              <TabsTrigger value="backend" className="text-xs sm:text-sm dark:data-[state=active]:bg-blue-600 dark:data-[state=active]:text-white">Back-end</TabsTrigger>
            </TabsList>
          </div>
